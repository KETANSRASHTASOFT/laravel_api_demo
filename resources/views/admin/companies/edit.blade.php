  ...
  <section class="content">
    ...
    <div class="card">
      <div class="card-header">
        <h3 class="card-title">{{ __('companies.companie_edit') }} : {{ $companie->name }}</h3>
      ...
      <form class="form-horizontal" method="POST" enctype="multipart/form-data" action="{{ route('companies.update',$companie->companies_id) }}">
        {{csrf_field()}}
        {{ method_field('PATCH') }}
        <div class="card-body">
          @if ($errors->any())
          ...
          <div class="form-group row">
            ...
            <div class="col-sm-6">  
              <input type="text" class="form-control @error('name') is-invalid @enderror" name="name" id="name" value="{{ $companie->name }}" placeholder="Compnay Name">
              @error('name')
          ...
          <div class="form-group row">
            ...
            <div class="col-sm-6">  
              <input type="email" class="form-control @error('email') is-invalid @enderror" name="email" id="email" placeholder="Compnay Name" value="{{ $companie->email }}">
              @error('email')
                <span class="invalid-feedback" role="alert">
                  <strong>{{ $message }}</strong>
                </span>
          ...
          <div class="form-group row">
            ...
            <div class="col-sm-6">
              @if($companie->logo == '')
                <img src="https://via.placeholder.com/100" data-src="https://via.placeholder.com/100" id="selectLogo" height="100" style="cursor: pointer;">
              @else
                <img src="{{ $companie->logo }}" height="100" data-src="{{ $companie->logo }}" id="selectLogo" style="cursor: pointer;">
              @endif  
          ...
          <div class="form-group row">
            ...
            <div class="col-sm-6">  
              <input type="text" class="form-control" name="website" id="website" placeholder="Compnay website" value="{{ $companie->website }}">
            </div>
          </div>
          <div class="form-group row">
            <label for="address" class="col-sm-2 col-form-label">{{ __('companies.address') }}</label>
            <div class="col-sm-6">  
              <textarea class="form-control" name="address" id="address" placeholder="Compnay Address">{{ $companie->address }}</textarea>
...
<script type="text/javascript">
  ...
  $(document).on("click","#clear",function(e){
    var dataSrc = $('#selectLogo').attr('data-src');
    $('#selectLogo').attr('src',dataSrc);
  });
